...
function isCOD(payload) {
  ...
  const names = (payload?.payment_gateway_names ?? []).map((n) => String(n).toLowerCase());
  return (
...
function normalizePhone(raw) {
  ...
  if (phone.startsWith("+")) return /^\+[1-9]\d{6,14}$/.test(phone) ? phone : null;
...
export const action = async ({ request }) => {
  ...
  if (!input.phoneNumber) {
    console.warn(`[OrderCreate] Missing valid phone for shopifyOrderId=${input.shopifyOrderId}`);
  ...
  try {
    ...
    console.log(
      `[OrderCreate] Dial started orderId=${order.id} callLogId=${callLog.id} vapiCallId=${vapiRes?.id ?? "n/a"}`,
    );
  } catch (err) {
    console.error(`[OrderCreate] triggerOrderConfirmationCall failed orderId=${order.id}`, err);
    ...
    if (isPermanentOrderVapiError(err)) {
      ...
      }).catch((e) => console.error("[OrderCreate] handleCallResult permanent failed", e));
    } else {
      await handleCallResult(order.id, CALL_INTENT.RECALL_REQUEST, {
        callLogId: callLog.id,
        failureReason: err.message,
      }).catch((e) => console.error("[OrderCreate] handleCallResult transient failed", e));
    }
  }
