...
function pushPayload(type, vapiId, result) {
    g.__vapiPayloads.unshift({ time: new Date().toLocaleTimeString(), type, vapiId, result });
    if (g.__vapiPayloads.length > 20) g.__vapiPayloads.pop();
}

function extractEndedReason(body) {
  const reason = body?.message?.endedReason ?? body?.endedReason ?? body?.message?.call?.endedReason;
  if (!reason) return null;

  const r = String(reason).toLowerCase();
  if (r.includes("customer-did-not-answer") || r.includes("no-answer")) return "recall_request";
...
function extractTranscript(body) {
  return body?.message?.artifact?.transcript
    || body?.message?.transcript
    || body?.transcript
    || body?.message?.artifact?.messages?.map(m => m?.content || m?.text).filter(Boolean).join(" ")
    || null;
}

async function pollVapi(orderId, callLogId, vapiId) {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    if (intent) {
        ...
        await handleCallResult(orderId, intent, { callLogId, vapiCallId: vapiId });
        return;
    }
  }

  console.log(`[VapiPoll] 🛑 No intent found for ${vapiId}. Scheduling retry...`);
...
export async function action({ request }) {
  ...
  if (immediate && ASSISTANT_INTENT_MAP[immediate]) {
    ...
    await handleCallResult(ctx.orderId, intent, { callLogId: ctx.callLogId, vapiCallId: vapiId });
    return Response.json({ ok: true });
  }
  ...
  if (TERMINAL_EVENTS.has(type)) {
    ...
    if (endedIntent) {
      console.log(`[VapiWebhook][${logId}] 📵 Ended without connection: ${endedIntent}`);
      pushPayload("ENDED_NO_CONNECT", vapiId, endedIntent);
      await handleCallResult(ctx.orderId, endedIntent, { callLogId: ctx.callLogId, vapiCallId: vapiId });
      return Response.json({ ok: true });
    }
    ...
    if (quickIntent) {
      ...
      await handleCallResult(ctx.orderId, quickIntent, { callLogId: ctx.callLogId, vapiCallId: vapiId });
      return Response.json({ ok: true });
    }
...
export async function loader() {
  return Response.json({ 
    status: "Vapi Order Webhook Active", 
    time: new Date().toISOString(),
    recent_payloads: g.__vapiPayloads
  });
}
